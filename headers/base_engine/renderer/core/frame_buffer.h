...
struct __attribute__((deprecated("gl_fbo_texture is deprecated", "frame_buffer"))) gl_fbo_texture
{
...
namespace detail
{
struct depth_texture_extension
{
  base_texture2d depth_texture;
};
struct empty_depth_texture_extension
{
};
struct depth_stencil_texture_extension
{
  base_texture2d depth_stencil_texture;
};
struct empty_depth_stencil_texture_extension
{
};
} // namespace detail

enum struct frame_buffer_option
{
  has_depth_texture,
  has_depth_stencil_texture
};

template <frame_buffer_option... TOptions>
struct frame_buffer_options
{
  constexpr static bool has_depth         = ((TOptions == frame_buffer_option::has_depth_texture) || ...);
  constexpr static bool has_depth_stencil = ((TOptions == frame_buffer_option::has_depth_stencil_texture) || ...);
};

struct color_attachement
{
...
{
  std::vector<base_texture2d> color_attachement_tex_handles;
  std::vector<base_texture2d::texture_format> _texture_format_cache{};

  constexpr static std::tuple color_attachments{TColorAttachments...};
  ...
  initialize(u32 _screen_w, u32 _screen_h)
  {
    ...
    color_attachement_tex_handles.reserve(sizeof...(TColorAttachments));

    glGenFramebuffers(1, &handle);
    glBindFramebuffer(GL_FRAMEBUFFER, handle);

    usize col_attachement_index = 0;
    for_constexpr<0, sizeof...(TColorAttachments), 1>(
        [&](auto ite)
        {
          ...
          glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + col_attachement_index, GL_TEXTURE_2D, curr_tex.handle, 0);
          ++col_attachement_index;
        });

    if constexpr (TOptions.has_depth)
    {
      this->depth_texture.initialize(_screen_w, _screen_h, base_texture2d::texture_format::depth);
      glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, this->depth_texture.handle, 0);
      LOG(INFO) << "DEBUG: current framebuffer object has depth attachement";
    }
    if constexpr (TOptions.has_depth_stencil)
    {
      this->depth_stencil_texture.initialize(_screen_w, _screen_h, base_texture2d::texture_format::depth_stencil);
      glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_TEXTURE_2D, this->depth_stencil_texture.handle, 0);
      LOG(INFO) << "DEBUG: current framebuffer object has depth_stencil attachement";
    }
  ...
  destroy()
  {
    for (auto& tex : color_attachement_tex_handles)
    {
      tex.destroy();
    }
    color_attachement_tex_handles.clear();
    ...
    if constexpr (TOptions.has_depth_stencil)
    {
      this->depth_stencil_texture.destroy();
    }
  ...
  u0
  resize(u32 _new_screen_w, u32 _new_screen_h)
  {
    bind_screen_buffer();
    destroy();
    initialize(_new_screen_w, _new_screen_h);
  }
