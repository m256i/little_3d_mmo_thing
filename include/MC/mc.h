...
static inline MC_FLOAT
mc_internalLength2(const mcVec3f& v)
{
  return v.x * v.x + v.y * v.y + v.z * v.z;
}
static inline MC_FLOAT
mc_internalLength(const mcVec3f& v)
{
  return std::sqrt(mc_internalLength2(v));
}
static inline mcVec3f
mc_internalNormalize(const mcVec3f& v)
{
  MC_FLOAT vv = mc_internalLength(v);
  return mcVec3f({{{v.x / vv, v.y / vv, v.z / vv}}});
}
static inline mcVec3f
mc_internalCross(const mcVec3f& v1, const mcVec3f& v2)
{
  return mcVec3f({{{v1.y * v2.z - v1.z * v2.y, v1.z * v2.x - v1.x * v2.z, v1.x * v2.y - v1.y * v2.x}}});
}
inline mcVec3f
operator-(const mcVec3f& l, const mcVec3f r)
{
...
static inline muint
mc_internalToIndex1D(muint i, muint j, muint k, const mcVec3i& size)
{
  return (k * size.y + j) * size.x + i;
}

static inline muint
mc_internalToIndex1DSlab(muint i, muint j, muint k, const mcVec3i& size)
{
...
*/
static void
mc_internalComputeEdge(mcVec3i* slab_inds, mcMesh& mesh, MC_FLOAT va, MC_FLOAT vb, int axis, muint x, muint y, muint z, const mcVec3i& size)
{
...
*/
static inline void
mc_internalAccumulateNormal(mcMesh& mesh, muint a, muint b, muint c)
{
...
*/
inline void
setDefaultArraySizes(muint vertSize, muint normSize, muint triSize)
{
...
*/
void
marching_cube(MC_FLOAT* field, muint nx, muint ny, muint nz, mcMesh& outputMesh)
{
  ...
  for (muint z = 0; z < nz - 1; z++)
  {
    for (muint y = 0; y < ny - 1; y++)
    {
      for (muint x = 0; x < nx - 1; x++)
      {
        ...
        const int config_n = ((vs[0] < 0) << 0) | ((vs[1] < 0) << 1) | ((vs[2] < 0) << 2) | ((vs[3] < 0) << 3) | ((vs[4] < 0) << 4) |
                             ((vs[5] < 0) << 5) | ((vs[6] < 0) << 6) | ((vs[7] < 0) << 7);
        ...
        {
          mc_internalAccumulateNormal(outputMesh, outputMesh.indices[indexBase + i * 3 + 0], outputMesh.indices[indexBase + i * 3 + 1],
