                               ...
                               std::string_view _tess_path_eval, const char *_geo_path)
{
  ...
  auto uniforms = this->get_uniforms();

  for (const auto &ite : uniforms)
  {
    this->hashed_uniform_handles[fnv1a::hash(ite.name.c_str())] = glGetUniformLocation(id, ite.name.c_str());
  }

  auto textures = this->get_input_textures();

  for (const auto &ite : textures)
  {
    this->hashed_uniform_handles[fnv1a::hash(ite.c_str())] = glGetUniformLocation(id, ite.c_str());
  }
