...
// size.
static void*
defaultReallocate(void* ptr, size_t newSize, void* _)
{
  if (newSize == 0)
  {
    free(ptr);
...
void
wrenInitConfiguration(WrenConfiguration* config)
{
...
WrenVM*
wrenNewVM(WrenConfiguration* config)
{
...
wrenCollectGarbage(WrenVM* vm)
{
  ...
  printf("GC %lu before, %lu after (%lu collected), next at %lu. Took %.3fms.\n", (unsigned long)before, (unsigned long)vm->bytesAllocated,
         (unsigned long)(before - vm->bytesAllocated), (unsigned long)vm->nextGC, elapsed * 1000.0);
#endif
}

void*
wrenReallocate(WrenVM* vm, void* memory, size_t oldSize, size_t newSize)
{
...
static ObjUpvalue*
captureUpvalue(WrenVM* vm, ObjFiber* fiber, Value* local)
{
...
// and above.
static void
closeUpvalues(ObjFiber* fiber, Value* last)
{
...
findForeignMethod(WrenVM* vm, const char* moduleName, const char* className, bool isStatic, const char* signature)
{
...
// found.
static void
bindMethod(WrenVM* vm, int methodType, int symbol, ObjModule* module, ObjClass* classObj, Value methodValue)
{
  ...
  if (IS_STRING(methodValue))
  {
    ...
    {
      vm->fiber->error = wrenStringFormat(vm, "Could not find foreign method '@' for class $ in module '$'.", methodValue,
...
static void
callForeign(WrenVM* vm, ObjFiber* fiber, WrenForeignMethodFn foreign, int numArgs)
{
...
static void
methodNotFound(WrenVM* vm, ObjClass* classObj, int symbol)
{
...
static ObjModule*
getModule(WrenVM* vm, Value name)
{
...
static ObjClosure*
compileInModule(WrenVM* vm, Value name, const char* source, bool isExpression, bool printErrors)
{
...
// error message.
static Value
validateSuperclass(WrenVM* vm, Value name, Value superclassValue, int numFields)
{
  ...
  if (superclass == vm->classClass || superclass == vm->fiberClass || superclass == vm->fnClass || // Includes OBJ_CLOSURE.
      ...
      superclass == vm->boolClass || superclass == vm->nullClass || superclass == vm->numClass)
  {
...
static void
bindForeignClass(WrenVM* vm, ObjClass* classObj, ObjModule* module)
{
...
static void
createClass(WrenVM* vm, int numFields, ObjModule* module)
{
...
static void
createForeign(WrenVM* vm, ObjFiber* fiber, Value* stack)
{
...
void
wrenFinalizeForeign(WrenVM* vm, ObjForeign* foreign)
{
  // TODO: Don't look up every time.
...
static Value
resolveModule(WrenVM* vm, Value name)
{
...
static Value
importModule(WrenVM* vm, Value name)
{
...
static Value
getModuleVariable(WrenVM* vm, ObjModule* module, Value variableName)
{
...
inline static bool
checkArity(WrenVM* vm, Value value, int numArgs)
{
...
runInterpreter(WrenVM* vm, ObjFiber* fiber)
{
...
WrenHandle*
wrenMakeCallHandle(WrenVM* vm, const char* signature)
{
...
WrenInterpretResult
wrenCall(WrenVM* vm, WrenHandle* method)
{
...
WrenHandle*
wrenMakeHandle(WrenVM* vm, Value value)
{
...
void
wrenReleaseHandle(WrenVM* vm, WrenHandle* handle)
{
...
ObjClosure*
wrenCompileSource(WrenVM* vm, const char* module, const char* source, bool isExpression, bool printErrors)
{
...
Value
wrenGetModuleVariable(WrenVM* vm, Value moduleName, Value variableName)
{
...
Value
wrenFindVariable(WrenVM* vm, ObjModule* module, const char* name)
{
...
int
wrenDeclareVariable(WrenVM* vm, ObjModule* module, const char* name, size_t length, int line)
{
...
int
wrenDefineVariable(WrenVM* vm, ObjModule* module, const char* name, size_t length, Value value, int* line)
{
  if (module->variables.count == MAX_MODULE_VARS) return -2;
...
// TODO: Inline?
void
wrenPushRoot(WrenVM* vm, Obj* obj)
{
...
static void
validateApiSlot(WrenVM* vm, int slot)
{
...
WrenType
wrenGetSlotType(WrenVM* vm, int slot)
{
...
bool
wrenGetSlotBool(WrenVM* vm, int slot)
{
  validateApiSlot(vm, slot);
...
const char*
wrenGetSlotBytes(WrenVM* vm, int slot, int* length)
{
  validateApiSlot(vm, slot);
...
double
wrenGetSlotDouble(WrenVM* vm, int slot)
{
  validateApiSlot(vm, slot);
...
void*
wrenGetSlotForeign(WrenVM* vm, int slot)
{
  validateApiSlot(vm, slot);
...
const char*
wrenGetSlotString(WrenVM* vm, int slot)
{
  validateApiSlot(vm, slot);
  ASSERT(IS_STRING(vm->apiStack[slot]), "Slot must hold a string.");
...
WrenHandle*
wrenGetSlotHandle(WrenVM* vm, int slot)
{
  validateApiSlot(vm, slot);
...
static void
setSlot(WrenVM* vm, int slot, Value value)
{
  validateApiSlot(vm, slot);
...
void
wrenSetSlotBool(WrenVM* vm, int slot, bool value)
{
  setSlot(vm, slot, BOOL_VAL(value));
}

void
wrenSetSlotBytes(WrenVM* vm, int slot, const char* bytes, size_t length)
{
...
void
wrenSetSlotDouble(WrenVM* vm, int slot, double value)
{
  setSlot(vm, slot, NUM_VAL(value));
}

void*
wrenSetSlotNewForeign(WrenVM* vm, int slot, int classSlot, size_t size)
{
  validateApiSlot(vm, slot);
...
void
wrenSetSlotNewList(WrenVM* vm, int slot)
{
  setSlot(vm, slot, OBJ_VAL(wrenNewList(vm, 0)));
}

void
wrenSetSlotNewMap(WrenVM* vm, int slot)
{
  setSlot(vm, slot, OBJ_VAL(wrenNewMap(vm)));
}

void
wrenSetSlotNull(WrenVM* vm, int slot)
{
  setSlot(vm, slot, NULL_VAL);
}

void
wrenSetSlotString(WrenVM* vm, int slot, const char* text)
{
...
void
wrenSetSlotHandle(WrenVM* vm, int slot, WrenHandle* handle)
{
  ASSERT(handle != NULL, "Handle cannot be NULL.");
...
int
wrenGetListCount(WrenVM* vm, int slot)
{
  validateApiSlot(vm, slot);
...
void
wrenGetListElement(WrenVM* vm, int listSlot, int index, int elementSlot)
{
...
void
wrenSetListElement(WrenVM* vm, int listSlot, int index, int elementSlot)
{
  validateApiSlot(vm, listSlot);
  validateApiSlot(vm, elementSlot);
  ASSERT(IS_LIST(vm->apiStack[listSlot]), "Slot must hold a list.");
...
void
wrenInsertInList(WrenVM* vm, int listSlot, int index, int elementSlot)
{
  validateApiSlot(vm, listSlot);
  validateApiSlot(vm, elementSlot);
...
int
wrenGetMapCount(WrenVM* vm, int slot)
{
  validateApiSlot(vm, slot);
...
bool
wrenGetMapContainsKey(WrenVM* vm, int mapSlot, int keySlot)
{
...
void
wrenGetMapValue(WrenVM* vm, int mapSlot, int keySlot, int valueSlot)
{
  validateApiSlot(vm, mapSlot);
  validateApiSlot(vm, keySlot);
...
void
wrenSetMapValue(WrenVM* vm, int mapSlot, int keySlot, int valueSlot)
{
  validateApiSlot(vm, mapSlot);
  validateApiSlot(vm, keySlot);
  validateApiSlot(vm, valueSlot);
...
void
wrenRemoveMapValue(WrenVM* vm, int mapSlot, int keySlot, int removedValueSlot)
{
  validateApiSlot(vm, mapSlot);
  validateApiSlot(vm, keySlot);
  ASSERT(IS_MAP(vm->apiStack[mapSlot]), "Slot must hold a map.");
...
void
wrenGetVariable(WrenVM* vm, const char* module, const char* name, int slot)
{
...
bool
wrenHasVariable(WrenVM* vm, const char* module, const char* name)
{
  ASSERT(module != NULL, "Module cannot be NULL.");
  ASSERT(name != NULL, "Variable name cannot be NULL.");
...
bool
wrenHasModule(WrenVM* vm, const char* module)
{
  ASSERT(module != NULL, "Module cannot be NULL.");
...
void
wrenAbortFiber(WrenVM* vm, int slot)
{
  validateApiSlot(vm, slot);
...
void*
wrenGetUserData(WrenVM* vm)
{
  return vm->config.userData;
}

void
wrenSetUserData(WrenVM* vm, void* userData)
{
