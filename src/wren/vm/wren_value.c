...
static void
initObj(WrenVM* vm, Obj* obj, ObjType type, ObjClass* classObj)
{
  obj->type     = type;
  obj->isDark   = false;
...
ObjClass*
wrenNewSingleClass(WrenVM* vm, int numFields, ObjString* name)
{
...
void
wrenBindSuperclass(WrenVM* vm, ObjClass* subclass, ObjClass* superclass)
{
...
ObjClass*
wrenNewClass(WrenVM* vm, ObjClass* superclass, int numFields, ObjString* name)
{
...
void
wrenBindMethod(WrenVM* vm, ObjClass* classObj, int symbol, Method method)
{
...
ObjClosure*
wrenNewClosure(WrenVM* vm, ObjFn* fn)
{
...
ObjFiber*
wrenNewFiber(WrenVM* vm, ObjClosure* closure)
{
...
void
wrenEnsureStack(WrenVM* vm, ObjFiber* fiber, int needed)
{
...
ObjForeign*
wrenNewForeign(WrenVM* vm, ObjClass* classObj, size_t size)
{
...
ObjFn*
wrenNewFunction(WrenVM* vm, ObjModule* module, int maxSlots)
{
...
void
wrenFunctionBindName(WrenVM* vm, ObjFn* fn, const char* name, int length)
{
...
Value
wrenNewInstance(WrenVM* vm, ObjClass* classObj)
{
...
ObjList*
wrenNewList(WrenVM* vm, uint32_t numElements)
{
...
void
wrenListInsert(WrenVM* vm, ObjList* list, Value value, uint32_t index)
{
...
int
wrenListIndexOf(WrenVM* vm, ObjList* list, Value value)
{
...
Value
wrenListRemoveAt(WrenVM* vm, ObjList* list, uint32_t index)
{
...
// should be inserted.
static bool
findEntry(MapEntry* entries, uint32_t capacity, Value key, MapEntry** result)
{
...
static bool
insertEntry(MapEntry* entries, uint32_t capacity, Value key, Value value)
{
...
static void
resizeMap(WrenVM* vm, ObjMap* map, uint32_t capacity)
{
...
Value
wrenMapGet(ObjMap* map, Value key)
{
  MapEntry* entry;
...
void
wrenMapSet(WrenVM* vm, ObjMap* map, Value key, Value value)
{
...
void
wrenMapClear(WrenVM* vm, ObjMap* map)
{
  DEALLOCATE(vm, map->entries);
  map->entries  = NULL;
  map->capacity = 0;
...
Value
wrenMapRemoveKey(WrenVM* vm, ObjMap* map, Value key)
{
  MapEntry* entry;
...
ObjModule*
wrenNewModule(WrenVM* vm, ObjString* name)
{
...
Value
wrenNewRange(WrenVM* vm, double from, double to, bool isInclusive)
{
...
// hash.
static ObjString*
allocateString(WrenVM* vm, size_t length)
{
...
Value
wrenNewString(WrenVM* vm, const char* text)
{
  return wrenNewStringLength(vm, text, strlen(text));
}

Value
wrenNewStringLength(WrenVM* vm, const char* text, size_t length)
{
...
Value
wrenNewStringFromRange(WrenVM* vm, ObjString* source, int start, uint32_t count, int step)
{
...
Value
wrenNumToString(WrenVM* vm, double value)
{
...
Value
wrenStringFromCodePoint(WrenVM* vm, int value)
{
...
Value
wrenStringFromByte(WrenVM* vm, uint8_t value)
{
  int length        = 1;
  ObjString* string = allocateString(vm, length);
...
Value
wrenStringFormat(WrenVM* vm, const char* format, ...)
{
  va_list argList;
...
Value
wrenStringCodePointAt(WrenVM* vm, ObjString* string, uint32_t index)
{
...
uint32_t
wrenStringFind(ObjString* haystack, ObjString* needle, uint32_t start)
{
...
ObjUpvalue*
wrenNewUpvalue(WrenVM* vm, Value* value)
{
...
void
wrenGrayValue(WrenVM* vm, Value value)
{
...
void
wrenGrayBuffer(WrenVM* vm, ValueBuffer* buffer)
{
...
static void
blackenClass(WrenVM* vm, ObjClass* classObj)
{
  // The metaclass.
...
static void
blackenClosure(WrenVM* vm, ObjClosure* closure)
{
  // Mark the function.
...
static void
blackenFiber(WrenVM* vm, ObjFiber* fiber)
{
  // Stack functions.
  for (int i = 0; i < fiber->numFrames; i++)
  {
...
static void
blackenForeign(WrenVM* vm, ObjForeign* foreign)
{
...
static void
blackenInstance(WrenVM* vm, ObjInstance* instance)
{
...
static void
blackenList(WrenVM* vm, ObjList* list)
{
  // Mark the elements.
...
static void
blackenMap(WrenVM* vm, ObjMap* map)
{
  // Mark the entries.
  for (uint32_t i = 0; i < map->capacity; i++)
  {
...
static void
blackenModule(WrenVM* vm, ObjModule* module)
{
  // Top-level variables.
...
static void
blackenRange(WrenVM* vm, ObjRange* range)
{
  // Keep track of how much memory is still in use.
...
static void
blackenString(WrenVM* vm, ObjString* string)
{
  // Keep track of how much memory is still in use.
...
static void
blackenUpvalue(WrenVM* vm, ObjUpvalue* upvalue)
{
...
static void
blackenObject(WrenVM* vm, Obj* obj)
{
#if WREN_DEBUG_TRACE_MEMORY
  printf("mark ");
...
ObjClass*
wrenGetClass(WrenVM* vm, Value value)
{
  return wrenGetClassInline(vm, value);
}

bool
wrenValuesEqual(Value a, Value b)
{
