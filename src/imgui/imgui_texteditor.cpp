...
#include <include/imgui/imgui.h> // for imGui::GetCurrentWindow()
...
TextEditor::TextEditor()
    : mLineSpacing(1.0f), mUndoIndex(0), mTabSize(4), mOverwrite(false), mReadOnly(false), mWithinRender(false), mScrollToCursor(false),
      mScrollToTop(false), mTextChanged(false), mColorizerEnabled(true), mTextStart(20.0f), mLeftMargin(10), mCursorPositionChanged(false),
      mColorRangeMin(0), mColorRangeMax(0), mSelectionMode(SelectionMode::Normal), mCheckComments(true), mLastClick(-1.0f),
...
TextEditor::SetSelection(const Coordinates& aStart, const Coordinates& aEnd, SelectionMode aMode)
{
  ...
  case TextEditor::SelectionMode::Word:
  {
    ...
    if (!IsOnWordBoundary(mState.mSelectionEnd)) mState.mSelectionEnd = FindWordEnd(FindWordStart(mState.mSelectionEnd));
    break;
  }
...
const TextEditor::LanguageDefinition&
TextEditor::LanguageDefinition::Wren()
{
  static bool inited = false;
  static LanguageDefinition langDef;
  if (!inited)
  {
    static const char* const cppKeywords[] = {
        "class", "is", "construct", "var", "static", "return", "for", "in", "import", "while", "if", "else", "super", "foreign", "this",
    };
    for (auto& k : cppKeywords)
      langDef.mKeywords.insert(k);

    static const char* const identifiers[] = {"null", "true", "false"};
    for (auto& k : identifiers)
    {
      Identifier id;
      id.mDeclaration = "";
...
TextEditor::LanguageDefinition::AngelScript()
{
  ...
  if (!inited)
  {
    ...
    static const char* const identifiers[] = {"cos",     "sin",      "tab",         "acos",        "asin",        "atan",        "atan2",
