...
//  2022-11-09: OpenGL: Reverted use of glBufferSubData(), too many corruptions issues + old issues seemingly can't be reproed with Intel drivers nowadays (revert 2021-12-15 and 2022-05-23
//  changes). 2022-10-11: Using 'nullptr' instead of 'NULL' as per our switch to C++11. 2022-09-27: OpenGL: Added ability to '#define IMGUI_IMPL_OPENGL_DEBUG'. 2022-05-23: OpenGL: Reworking
//  2021-12-15 "Using buffer orphaning" so it only happens on Intel GPU, seems to cause problems otherwise. (#4468, #4825, #4832, #5127). 2022-05-13: OpenGL: Fixed state corruption on OpenGL
//  ES 2.0 due to not preserving GL_ELEMENT_ARRAY_BUFFER_BINDING and vertex attribute states. 2021-12-15: OpenGL: Using buffer orphaning + glBufferSubData(), seems to fix leaks with
//  multi-viewports with some Intel HD drivers. 2021-08-23: OpenGL: Fixed ES 3.0 shader ("#version 300 es") use normal precision floats to avoid wobbly rendering at HD resolutions.
...
ImGui_ImplOpenGL3_Init(const char* glsl_version)
{
  ...
  */
  ...
  printf("GlVersion = %d\nGlProfileIsCompat = %d\nGlProfileMask = 0x%X\nGlProfileIsES2 = %d, GlProfileIsES3 = %d\nGL_VENDOR = '%s'\nGL_RENDERER = '%s'\n", bd->GlVersion,
         bd->GlProfileIsCompat, bd->GlProfileMask, bd->GlProfileIsES2, bd->GlProfileIsES3, (const char*)glGetString(GL_VENDOR), (const char*)glGetString(GL_RENDERER)); // [DEBUG]
#endif
...
ImGui_ImplOpenGL3_RenderDrawData(ImDrawData* draw_data)
{
  ...
  // This "glIsProgram()" check is required because if the program is "pending deletion" at the time of binding backup, it will have been deleted by now and will cause an OpenGL error. See
  // #6220.
...
ImGui_ImplOpenGL3_CreateFontsTexture()
{
  ...
  io.Fonts->GetTexDataAsRGBA32(
      &pixels, &width, &height); // Load as RGBA 32-bit (75% of the memory is wasted, but default font is so small) because it is more likely to be compatible with user's existing shaders.
