...
#else
static const char* kDefaultLogFile = "game_logs.log";
...
static struct StringToLevelItem stringToLevelMap[] = {{"global", Level::Global}, {"debug", Level::Debug}, {"info", Level::Info},       {"warning", Level::Warning},
...
bool
Configurations::Parser::parseFromFile(const std::string& configurationFile, Configurations* sender, Configurations* base)
{
...
bool
Configurations::Parser::parseFromText(const std::string& configurationsString, Configurations* sender, Configurations* base)
{
  sender->setFromBase(base);
  bool parsedSuccessfully = false;
...
bool
Configurations::Parser::parseLine(std::string* line, std::string* currConfigStr, std::string* currLevelStr, Level* currLevel, Configurations* conf)
{
...
void
Configurations::unsafeSetGlobally(ConfigurationType configurationType, const std::string& value, bool includeGlobalLevel)
{
  if (includeGlobalLevel)
  {
...
LogBuilder::convertToColoredOutput(base::type::string_t* logLine, Level level)
{
  ...
  if (level == Level::Error || level == Level::Fatal) *logLine = ELPP_LITERAL("\x1b[31m") + *logLine + resetColor;
...
void
Str::replaceFirstWithEscape(base::type::string_t& str, const base::type::string_t& replaceWhat, const base::type::string_t& replaceWith)
{
...
#if defined(ELPP_UNICODE)
void
Str::replaceFirstWithEscape(base::type::string_t& str, const base::type::string_t& replaceWhat, const std::string& replaceWith)
{
...
DateTime::gettimeofday(struct timeval* tv)
{
  ...
  if (tv != nullptr)
  {
    ...
    const unsigned long long delta_ = 11644473600000000Ui64;
#else
    const unsigned long long delta_ = 11644473600000000ULL;
...
unsigned long long
DateTime::getTimeDifference(const struct timeval& endTime, const struct timeval& startTime, base::TimestampUnit timestampUnit)
{
...
char*
DateTime::parseFormat(char* buf, std::size_t bufSz, const char* format, const struct tm* tInfo, std::size_t msec, const base::SubsecondPrecision* ssPrec)
{
...
LogFormat::LogFormat(const LogFormat& logFormat)
    : m_level(logFormat.m_level), m_userFormat(logFormat.m_userFormat), m_format(logFormat.m_format), m_dateTimeFormat(logFormat.m_dateTimeFormat), m_flags(logFormat.m_flags),
      m_currentUser(logFormat.m_currentUser), m_currentHost(logFormat.m_currentHost)
{
}
...
{
  base::threading::ScopedLock scopedLock(lock());
  if (level > 9) m_level = base::consts::kMaxVerboseLevel;
  else
    m_level = level;
}
...
Storage::Storage(const LogBuilderPtr& defaultLogBuilder)
    :
#endif // ELPP_ASYNC_LOGGING
      m_registeredHitCounters(new base::RegisteredHitCounters()), m_registeredLoggers(new base::RegisteredLoggers(defaultLogBuilder)), m_flags(ELPP_DEFAULT_LOGGING_FLAGS),
...
DefaultLogDispatchCallback::dispatch(base::type::string_t&& logLine)
{
  ...
  else if (m_data->dispatchAction() == base::DispatchAction::SysLog)
  {
    ...
    if (m_data->logMessage()->level() == Level::Fatal) sysLogPriority = LOG_EMERG;
...
AsyncDispatchWorker::handle(AsyncLogItem* logItem)
{
  ...
  else if (data->dispatchAction() == base::DispatchAction::SysLog)
  {
    // Determine syslog priority
    int sysLogPriority = 0;
    if (logMessage->level() == Level::Fatal) sysLogPriority = LOG_EMERG;
...
PerformanceTracker::PerformanceTracker(const std::string& blockName, base::TimestampUnit timestampUnit, const std::string& loggerId, bool scopedLog, Level level)
    : m_blockName(blockName), m_timestampUnit(timestampUnit), m_loggerId(loggerId), m_scopedLog(scopedLog), m_level(level), m_hasChecked(false), m_lastCheckpointId(std::string()),
      m_enabled(false)
{
